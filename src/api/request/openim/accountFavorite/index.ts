...
import {
    ...
    getAccountfavorite,
    delAccountfavorite,
} from "@/api/openim/accountFavorite";
...
export default () => {
    ...
    const getList = () => {
        ...
        loading.value = true;
        // prettier-ignore
        listAccountfavorite(queryParams.value).then((response) => {
    ...
    const handleUpdate = (row: any) => {
        ...
        getAccountfavorite(configId).then((response) => {
    ...
    const handleDelete = (row: any) => {
        ...
        proxy.$modal.confirm("是否确认删除编号为\"" + configIds + "\"的数据项?", "警告")
            .then(() => {
                return delAccountfavorite(query.value);
            })
            .then((response: any) => {
                if (response.code === 200) {
                    getList();
