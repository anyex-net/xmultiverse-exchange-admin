...
// prettier-ignore
import { listSnsPost ,getSnsPost,delSnsPost} from "@/api/social/snsPost";
...
export default () => {
    ...
    const getList = () => { for (let key in queryParams.value) {
        ...
        loading.value = true;
        // prettier-ignore
        listSnsPost(queryParams.value).then((response) => {
    ...
    const handleUpdate = (row: any) => {
        reset();
        getSnsPost(row.id).then((res:any)=>{
            if (res.code==200){
               form.value=res.data;
            }
        })
