...
// prettier-ignore
import { listSnsPostLike ,getSnsPostLike,delSnsPostLike} from "@/api/social/snsPostLike";
...
export default () => {
    ...
    const getList = () => { for (let key in queryParams.value) {
        ...
        loading.value = true;
        // prettier-ignore
        listSnsPostLike(queryParams.value).then((response) => {
    ...
    const handleUpdate = (row: any) => {
        reset();
        getSnsPostLike(row.id).then((res:any)=>{
            if (res.code==200){
                form.value=res.data;
            }
        })
    ...
    const handleDelete = (row: any) => {
        ...
        proxy.$modal.confirm('是否确认删除id为' + arrIds + '的数据项?').then(() => {
            ...
            return delSnsPostLike(query.value)
        }).then((response: any) => {
            if (response.code === 200) {
