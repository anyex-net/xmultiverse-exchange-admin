import {
  ...
  export default () => {
    ...
    const getList = () => {
      ...
      loading.value = true;
      // prettier-ignore
      listfriendList(queryParams.value).then((response:any) => {
    ...
    const handleUpdate = (row: any) => {
      ...
      getfriendList(configId).then((response: any) => {
    ...
    const submitForm = async () => {
      await formRef.value?.validate((valid: boolean) => {
        if (valid) {
          if (form.value.id != "") {
            addfriendList(form.value).then((response) => {
              if (response.code === 200) {
    ...
    const handleDelete = (row: any) => {
      ...
      proxy.$modal.confirm("是否确认删除编号为\"" + configIds + "\"的数据项?", "警告")
              .then(() => {
                  return delfriendList(query.value);
              })
              .then((response: any) => {
                  if (response.code === 200) {
                      getList();
