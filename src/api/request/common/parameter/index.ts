...
// prettier-ignore
import { listParameter, getParameter, delParameter, addParameter } from "@/api/common/parameter";
import { ElForm, ElTable } from "element-plus";
import { isStrings } from "@/utils/validate";

export default () => {
    const { proxy } = getCurrentInstance() as any;
    const state = reactive<parameter<parameterForm, parameterQueryParams>>({
        // 查询参数
        queryParams: {
            current: 1,
            size: 50,
    ...
    const { queryParams, loading, ids, single, multiple, showSearch, total, configList, title, open, form, rules } = toRefs(state);
    ...
    const handleUpdate = (row: any) => {
        ...
        getParameter(configId).then((response) => {
    ...
    const handleDelete = (row: any) => {
        ...
        proxy.$modal.confirm('是否确认删除参数编号为"' + configIds + '"的数据项?', "警告")
            .then(() => {
                return delParameter(query.value);
            })
            .then((response: any) => {
                if (response.code === 200) {
                    getList();
