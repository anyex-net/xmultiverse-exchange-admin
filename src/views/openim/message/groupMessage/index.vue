      ...
      >
        ...
        <el-table-column label="群组头像" prop="faceUrl">
          <template #default="scope">
            <img
              v-if="JSON.parse(scope.row.content).sourceUrl == undefined"
            ...
            <div v-else>
              ...
              <el-image
                v-else
                style="width: 30px; height: 30px; border-radius: 5px"
                :src="JSON.parse(scope.row.content).sourceUrl"
                :preview-src-list="[JSON.parse(scope.row.content).sourceUrl]"
                :initial-index="1"
                :z-index="99999"
        ...
        <el-table-column label="群人数" prop="groupMemberCount" />
        <el-table-column label="群主ID" prop="groupOwner" />
