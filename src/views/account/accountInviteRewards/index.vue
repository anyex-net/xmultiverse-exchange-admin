  ...
  <div class="app-container">
    <el-form size="small" :model="queryParams" ref="queryFormRef" :inline="true" v-show="showSearch"
      ...
      <el-form-item label="推荐码" prop="referralCode">
        <el-input v-model="queryParams.referralCode" placeholder="请输入" clearable style="width: 240px"
                  @keyup.enter.native="handleQuery()" @change="handleQuery()"/>
      </el-form-item>
      <!-- prettier-ignore -->
    ...
    总发放佣金<span v-html="Number(statisticsEntity.total).toFixed(2)"></span>&nbsp; &nbsp;今日发放佣金<span v-html="Number(statisticsEntity.curTotal).toFixed(2)"></span>
    <div class="self-table">
      <el-table size="small" stripe v-loading="loading" ref="pageTableRef" :data="configList"
        ...
        <el-table-column label="操作" min-width="90px" class-name="small-padding fixed-width" fixed="right">
          <template #default="scope">
            <el-link class="table_link_btn" :underline="false" type="primary"
                     @click="handleUpdate(scope.row)" v-hasPermi="['account:accountInviteRewards:operator']"><span
                class="table_link_text">账户关系</span></el-link>
          </template>
        </el-table-column>
      </el-table>
    </div>
    ...
    <el-dialog :title="title" v-model="open" width="500px" append-to-body @close="cleanSelect()">
      <el-form size="small" ref="formRef" :model="form" :rules="rules">
        ...
        <el-form-item label="推荐码" prop="referralCode" style="font-weight: 600;">
        ...
        <el-form-item label="一级邀请人数" prop="levelOneCnt" style="font-weight: 600;">
          <span class="mRbox">{{ form.levelOneCnt }}</span>
        </el-form-item>
        <el-form-item label="二级邀请人数" prop="levelTwoCnt" style="font-weight: 600;">
          <span class="mRbox">{{ form.levelTwoCnt }}</span>
        </el-form-item>
        <el-form-item label="三级邀请人数" prop="levelThreeCnt" style="font-weight: 600;">
...
import account from "@/api/request/account/accountInviteRewards/index";
