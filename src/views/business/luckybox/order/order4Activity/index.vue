    ...
    <div class="app-container">
        <el-form size="small" :model="queryParams" ref="queryFormRef" :inline="true" v-show="showSearch"
            ...
            <el-form-item label="是否开奖" prop="isLotteryDrawn">
                <el-select style="width: 215px" v-model="queryParams.isLotteryDrawn" @change="handleQuery"
                    placeholder="请选择" clearable>
        ...
        <div class="self-table">
            <el-table size="small" stripe v-loading="loading" ref="pageTableRef" :data="configList"
                ...
                <el-table-column label="活动ID" fixed prop="activityId" min-width="140px" />
                ...
                <el-table-column label="商品活动类型" prop="activityType" min-width="120px" />
                <el-table-column label="价格" prop="orderActPrice" min-width="100px">
                    <template #default="scope">
                        <span>{{ scope.row.orderActPrice.toFixed(2) }}</span>
                    </template>
                </el-table-column>
                <el-table-column label="尾款金额" prop="orderActBalancePayment" min-width="100px">
                    <template #default="scope">
                        <span>{{ scope.row.orderActBalancePayment.toFixed(2) }}</span>
                    </template>
                </el-table-column>
                <el-table-column label="活动原价" prop="activitySkuPrice" min-width="100px">
                    <template #default="scope">
                        <span>{{ scope.row.activitySkuPrice.toFixed(2) }}</span>
                    </template>
                </el-table-column>
                <el-table-column label="购买份数" prop="orderActPurchaseNum" min-width="100px" />
                <el-table-column label="总金额" prop="orderSumBalance" min-width="100px">
                ...
                <el-table-column label="订单状态" prop="orderStatus" min-width="120px">
                    ...
                    <template #default="scope">
                        ...
                        <span v-if="scope.row.orderStatus == '9'">已支付尾款但缺货等值现金充抵</span>
                    </template>
                </el-table-column>
                <el-table-column label="余额扣减金额" prop="balanceDeductionAmount" min-width="100px">

                    <template #default="scope">
                        <span v-if="scope.row.balanceDeductionAmount != ''">{{
                            scope.row.balanceDeductionAmount.toFixed(2) }}</span>
                    </template>
                </el-table-column>
                <!--
                ...
                -->
                <el-table-column label="一轮总份数" prop="activitySumNum" min-width="100px" />
                <el-table-column label="总轮数" prop="activitySumRound" min-width="100px" />
                <el-table-column label="当前轮数" prop="activityCurrentRound" min-width="100px" />
                <el-table-column label="是否开奖" prop="isLotteryDrawn" min-width="100px">
                ...
                <el-table-column label="操作" fixed="right" min-width="90px" class-name="small-padding fixed-width">
                    ...
                    <template #default="scope">
                        <el-link class="table_link_btn" :underline="false" type="primary"
                            @click="handleUpdate(scope.row)" v-hasPermi="['order:order4Activity:operator']"><span
        ...
        <el-dialog :title="title" v-model="open" width="500px" append-to-body @close="cleanSelect()">
            <el-form size="small" ref="formRef" :model="form" :rules="rules">
                ...
                <el-form-item label="商品活动类型" prop="activityType" style="font-weight: 600;">
                    <span class="mRbox">{{ form.activityType }}</span>
                </el-form-item>
                <el-form-item label="价格" prop="orderActPrice" style="font-weight: 600;">
                    <span class="mRbox">{{ Number(form.orderActPrice).toFixed(2) }}</span>
                </el-form-item>
                <el-form-item label="尾款金额" prop="orderActBalancePayment" style="font-weight: 600;">
                    <span class="mRbox">{{ Number(form.orderActBalancePayment).toFixed(2) }}</span>
                </el-form-item>
                <el-form-item label="活动原价" prop="activitySkuPrice" style="font-weight: 600;">
                    <span class="mRbox">{{ Number(form.activitySkuPrice).toFixed(2) }}</span>
                </el-form-item>
                <el-form-item label="购买份数" prop="orderActPurchaseNum" style="font-weight: 600;">
                    <span class="mRbox">{{ form.orderActPurchaseNum }}</span>
                </el-form-item>
                <el-form-item label="总金额" prop="orderSumBalance" style="font-weight: 600;">
                    <span class="mRbox">{{ Number(form.orderSumBalance).toFixed(2) }}</span>
                </el-form-item>
                <el-form-item label="状态" prop="orderStatus" style="font-weight: 600;">
                    ...
                    <span class="mRbox" v-if="form.orderStatus == '2'">已中奖无需支付尾款</span>
                    <span class="mRbox" v-if="form.orderStatus == '3'">已中奖待支付尾款</span>
                    <span class="mRbox" v-if="form.orderStatus == '4'">已支付尾款待发货</span>
                    <span class="mRbox" v-if="form.orderStatus == '5'">已发货待收货</span>
                    <span class="mRbox" v-if="form.orderStatus == '6'">已收货完成</span>
                    <span class="mRbox" v-if="form.orderStatus == '9'">已支付尾款但缺货等值现金充抵</span>
                </el-form-item>
                <el-form-item label="余额扣减金额" prop="balanceDeductionAmount" style="font-weight: 600;">
                    <span class="mRbox">{{ Number(form.balanceDeductionAmount).toFixed(2) }}</span>
                </el-form-item>
                ...
                <el-form-item label="一轮总份数" prop="activitySumNum" style="font-weight: 600;">
                    <span class="mRbox">{{ form.activitySumNum }}</span>
                </el-form-item>
                <el-form-item label="总轮数" prop="activitySumRound" style="font-weight: 600;">
                    <span class="mRbox">{{ form.activitySumRound }}</span>
                </el-form-item>
                <el-form-item label="当前轮数" prop="activityCurrentRound" style="font-weight: 600;">
                    <span class="mRbox">{{ form.activityCurrentRound }}</span>
                </el-form-item>
                <el-form-item label="是否开奖" prop="isLotteryDrawn" style="font-weight: 600;">
...
import order4Activity from "@/api/request/business/luckybox/order/order4Activity/index";
