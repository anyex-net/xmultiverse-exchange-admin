...
<!--        </el-form>-->
        ...
        <el-row :gutter="10" class="mb8">
            <el-col :span="1.5">
                <el-button type="primary" plain size="small" @click="handleAdd"
                           v-hasPermi="['system:organization:operator']">新增
                </el-button>
            </el-col>
            <el-col :span="1.5">
        ...
        <div class="self-table">
            <el-table size="small" stripe v-loading="loading" v-if="refreshTable" :data="deptList"
                      :default-expand-all="isExpandAll"
                      row-key="id" :tree-props="{ children: 'children', hasChildren: 'hasChildren' }">
                <el-table-column label="机构名称" prop="orgName" min-width="120" />

                <el-table-column label="机构描述" min-width="120" prop="orgDest" :show-overflow-tooltip="true" />
<!--                <el-table-column label="创建人" min-width="120" prop="createByName" :show-overflow-tooltip="true" />-->
                ...
                <el-table-column label="操作" align="center" width="200" class-name="small-padding fixed-width">
                    <template #default="scope">
                        <el-link class="table_link_btn" :underline="false" type="primary"
                                 @click="handleUpdate(scope.row)"><span
                            class="table_link_text" v-hasPermi="['system:organization:operator']">修改</span></el-link>
                        <el-link class="table_link_btn" :underline="false" type="primary"
                                 @click="handleAdd(scope.row)"><span
                            class="table_link_text" v-hasPermi="['system:organization:operator']">新增</span></el-link>

                        <el-link v-if="scope.row.parentId!=0" class="table_link_btn" :underline="false" size="small"
                                 type="primary" @click="handleDelete(scope.row)"
                                 v-hasPermi="['system:organization:operator']"><span
        ...
        <el-dialog :title="title" v-model="open" width="30%" append-to-body>
            <el-form size="small" ref="formRef" :model="form" :rules="rules" label-width="80px">
                <el-form-item v-if="form.parentId!==0" label="上级机构" prop="parentId">
                    <!-- prettier-ignore -->
                ...
                <el-form-item label="机构描述" prop="orgDest">
                    <el-input v-model="form.orgDest" placeholder="请输入" type="textarea" />
                </el-form-item>
            </el-form>
            <template #footer>
...
import Organization from "@/api/request/system/organization";
