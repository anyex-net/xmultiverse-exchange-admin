        ...
        <el-row :gutter="10" class="mb8">
            <el-col :span="1.5">
                <el-button type="primary" plain size="small" @click="handleAdd"
                           v-hasPermi="['common:dictionary:operator']">新增
                </el-button>
            </el-col>
            <el-col :span="1.5">
            ...
            <el-table size="small" stripe v-loading="loading" v-if="refreshTable" :data="deptList"
                ...
                <el-table-column label="编码" min-width="120" prop="code"  >
                    <template #default="scope">
                        <el-tooltip :disabled="isShowTooltip" :content="scope.row.code" placement="top">
                            <!-- 单行省略样式、鼠标移入事件 -->
                            <div class="singe-line" @mouseover="onMouseOver($event.target)">{{ scope.row.code }}</div>
                        </el-tooltip>
                    </template>
                </el-table-column>
                ...
                <el-table-column label="操作"  min-width="150" class-name="small-padding fixed-width">
                    <template #default="scope">
                        <el-link class="table_link_btn" :underline="false" type="primary"
                            ...
                            class="table_link_text" v-hasPermi="['common:dictionary:operator']">修改</span></el-link>
                        <el-link class="table_link_btn" :underline="false" type="primary"
                                 @click="handleAdd(scope.row)"><span
                            class="table_link_text" v-hasPermi="['common:dictionary:operator']">新增</span></el-link>

                        <el-link  class="table_link_btn" :underline="false" size="small"
                                 type="primary" @click="handleDelete(scope.row)"
                                 v-hasPermi="['common:dictionary:operator']"><span
...
import Dictionary from "@/api/request/common/dictionary";
