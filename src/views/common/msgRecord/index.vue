        ...
        <div class="self-table">
            <el-table size="small" stripe  v-loading="loading" ref="pageTableRef" :data="operateList"
                ...
                <el-table-column label="发送对象" prop="object" min-width="120" >
                    <template #default="scope">
                        <el-tooltip :disabled="isShowTooltip" :content="scope.row.object" placement="top">
                            <!-- 单行省略样式、鼠标移入事件 -->
                            <div class="singe-line" @mouseover="onMouseOver($event.target)">{{ scope.row.object }}</div>
                        </el-tooltip>
                    </template>
                </el-table-column>
...
import Smslog from "@/api/request/common/msgRecord";
