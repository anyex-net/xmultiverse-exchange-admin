...
// prettier-ignore
import { listAccessLog, delAccessLog } from "@/api/system/accessLog";
import { ElForm, ElTable } from "element-plus";
import { isStrings } from "@/utils/validate";

export default () => {
    const { proxy } = getCurrentInstance() as any;
    const state = reactive<operatelog<accessLogQueryParams>>({
        // 查询参数
        queryParams: {
            current: 1,
            size: 50,
    ...
    const getList = () => { for (let key in queryParams.value) {
      if (queryParams.value.hasOwnProperty(key)) {
        ...
        loading.value = true;
        // prettier-ignore
        listAccessLog(proxy.addDateRange(queryParams.value)).then((response) => {
    ...
    const handleDelete = (row: any) => {
        ...
        // prettier-ignore
        proxy.$modal.confirm("是否确认删除访问日之编号为\"" + dictTypeIds + "\"的数据项?", "警告")
            .then(() => {
                return delAccessLog(query.value);
            })
            .then((response: any) => {
                if (response.code === 200) {
                    getList();
