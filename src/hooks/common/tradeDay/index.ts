...
// prettier-ignore
import { listtradeDay, gettradeDay, deltradeDay, addtradeDay } from "@/api/common/tradeDay";
...
export default () => {
  ...
  const getList = () => {
    ...
    loading.value = true;
    // prettier-ignore
    listtradeDay(queryParams.value).then((response) => {
  ...
  const handleUpdate = (row: any) => {
    ...
    gettradeDay(configId).then((response) => {
  ...
  const submitForm = async () => {
    await formRef.value?.validate((valid: boolean) => {
      if (valid) {
        if (form.value.id != "") {
          addtradeDay(form.value).then((response) => {
            if (response.code === 200) {
  ...
  const handleDelete = (row: any) => {
    ...
    proxy.$modal.confirm('是否确认删除编号为"' + configIds + '"的数据项?', "警告")
            .then(() => {
                return deltradeDay(query.value);
            })
            .then((response: any) => {
                if (response.code === 200) {
                    getList();
  ...
  return {
        ...
        getList, cancel, reset, handleQuery, resetQuery, handleAdd, handleSelectionChange,listA3, handleUpdate, submitForm, handleDelete, pageTableRef, cleanSelect,isShowTooltip, onMouseOver,
    };
};
