...
// prettier-ignore
import { listRegion, delRegion } from "@/api/common/region";
...
export default () => {
    ...
    const getList = () => { for (let key in queryParams.value) {
      if (queryParams.value.hasOwnProperty(key)) {
        ...
        loading.value = true;
        // prettier-ignore
        listRegion(proxy.addDateRange(queryParams.value)).then((response) => {
    ...
    const handleDelete = (row: any) => {
        ...
        // prettier-ignore
        proxy.$modal.confirm("是否确认删除区域代码编号为\"" + dictTypeIds + "\"的数据项?", "警告")
            .then(() => {
                return delRegion(query.value);
            })
            .then((response: any) => {
                if (response.code === 200) {
                    getList();
