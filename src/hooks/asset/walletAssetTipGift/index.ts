...
import {
      ...
      delgiftList,
      uploadPolicy,
  } from "@/api/asset/walletAssetTipGift";
...
export default () => {
  ...
  const getList = () => {
    ...
    loading.value = true;
    // prettier-ignore
    listgiftList(queryParams.value).then((response) => {
  ...
  const handleDetail1 = (row: any) => {
    ...
    getgiftList(configId).then((response: any) => {
  ...
  const submitForm = async () => {
    await formRef.value?.validate((valid: boolean) => {
      if (valid) {
        if (form.value.id != "") {
          addgiftList(form.value).then((response) => {
            if (response.code === 200) {
  ...
  const handleDelete = (row: any) => {
    ...
    proxy.$modal.confirm("是否确认删除编号为\"" + configIds + "\"的数据项?", "警告")
              .then(() => {
                  return delgiftList(query.value);
              })
              .then((response: any) => {
                  if (response.code === 200) {
                      getList();
