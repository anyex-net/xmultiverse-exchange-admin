...
// prettier-ignore
import { listSnsPostShare ,getSnsPostShare,delSnsPostShare} from "@/api/social/snsPostShare";
...
export default () => {
    ...
    const getList = () => { for (let key in queryParams.value) {
        ...
        loading.value = true;
        // prettier-ignore
        listSnsPostShare(queryParams.value).then((response) => {
    ...
    const handleUpdate = (row: any) => {
        reset();
        getSnsPostShare(row.id).then((res:any)=>{
            if (res.code==200){
                form.value=res.data;
            }
        })
    ...
    const handleDelete = (row: any) => {
        ...
        proxy.$modal.confirm('是否确认删除id为' + arrIds + '的数据项?').then(() => {
            ...
            return delSnsPostShare(query.value)
        }).then((response: any) => {
            if (response.code === 200) {
