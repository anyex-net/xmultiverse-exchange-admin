...
// prettier-ignore
import { listSnsPostComment ,getSnsPostComment,delSnsPostComment} from "@/api/social/snsPostComment";
...
export default () => {
    ...
    const getList = () => { for (let key in queryParams.value) {
        ...
        loading.value = true;
        // prettier-ignore
        listSnsPostComment(queryParams.value).then((response) => {
    ...
    const handleUpdate = (row: any) => {
        reset();
        getSnsPostComment(row.id).then((res:any)=>{
            if (res.code==200){
                form.value=res.data;
            }
        })
    ...
    const handleDelete = (row: any) => {
        ...
        proxy.$modal.confirm('是否确认删除id为' + arrIds + '的数据项?').then(() => {
            ...
            return delSnsPostComment(query.value)
        }).then((response: any) => {
            if (response.code === 200) {
