import {
  ...
  import {
      ...
      dellikeArr,
      uploadPolicy,
  } from "@/api/social/snsPostCommentLike";
  ...
  export default () => {
    ...
    const getList = () => {
      ...
      loading.value = true;
      // prettier-ignore
      listlikeArr(queryParams.value).then((response:any) => {
    ...
    const handleUpdate = (row: any) => {
      ...
      getlikeArr(configId).then((response: any) => {
    ...
    const submitForm = async () => {
      await formRef.value?.validate((valid: boolean) => {
        if (valid) {
          if (form.value.id != "") {
            addlikeArr(form.value).then((response) => {
              if (response.code === 200) {
