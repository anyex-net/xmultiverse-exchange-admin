...
import {
    ...
    checkDatas
    // allCurrencies,
} from "@/api/user/userHoldAmountRewardConfig";
import { isStrings } from "@/utils/validate";
import { formDefault, searchDefault } from "@/data/user/userHoldAmountRewardConfig";
...
export default () => {
        ...
        await listDatas(obj).then((response: any) => {
            ...
            if (response.code == 200) {
                dataList.value = response.data.records.map((i: any) => ({
                    ...i,
                    thawTime: i.thawTime
                        ? new Date(+i.thawTime).toLocaleString()
                        : "",
    ...
    const handleStatusChange = async (row:any) => {
        ...
        await proxy.$modal.confirm(`确认要调整为${text}吗?`, "警告")
            .then(async () => {
