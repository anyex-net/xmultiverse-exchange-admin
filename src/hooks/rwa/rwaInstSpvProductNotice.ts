...
import {
    ...
    checkData
    // allCurrencies,
} from "@/api/rwa/rwaInstSpvProductNotice";
import { isStrings } from "@/utils/validate";
import {
    formDefault,
    searchDefault,
} from "@/data/rwa/rwaInstSpvProductNotice";
...
export default () => {
    ...
    const handleStatusChange = async (row: any, action: any) => {
        ...
        if (action == 2) {
            ...
            text = "驳回";
            reason = await proxy.$prompt('请输入驳回理由', '提示', {
                confirmButtonText: '确定',
                cancelButtonText: '取消',
                inputPattern: /\S/, // 确保输入非空
        ...
        await proxy.$modal.confirm(`确认要${text} "${row.noticeTitle}"吗?`, "警告")
            .then(async () => {
                checkData({id: row.id, state: action,reason: reason}).then((res: any) => {
                    if (res.code === 200){
