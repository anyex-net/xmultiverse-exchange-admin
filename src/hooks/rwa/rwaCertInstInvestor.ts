...
import {
  ...
  checkDatas
  // allCurrencies,
} from "@/api/rwa/rwaCertInstInvestor";
import { isStrings } from "@/utils/validate";
import { formDefault, searchDefault } from "@/data/rwa/rwaCertInstInvestor";
    ...
    await listDatas(obj).then((response: any) => {
      ...
      if (response.code == 200) {
        dataList.value = response.data.records.map((i: any) => ({
          ...i,
          companyFoundedDate:i.companyFoundedDate
              ? new Date(+i.companyFoundedDate).toLocaleString()
              : "--",
