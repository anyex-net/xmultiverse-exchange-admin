...
// prettier-ignore
import {
    listAccountsignindetail,
    getAccountsignindetail,
    delAccountsignindetail,
} from "@/api/account/accountSignInDetail";
...
export default () => {
    ...
    const getList = () => {
        ...
        loading.value = true;
        // prettier-ignore
        listAccountsignindetail(queryParams.value).then((response) => {
    ...
    const handleUpdate = (row: any) => {
        ...
        getAccountsignindetail(configId).then((response) => {
    ...
    const handleDelete = (row: any) => {
        ...
        proxy.$modal.confirm("是否确认删除编号为\"" + configIds + "\"的数据项?", "警告")
            .then(() => {
                return delAccountsignindetail(query.value);
            })
            .then((response: any) => {
                if (response.code === 200) {
                    getList();
