...
// prettier-ignore
import {
    listActivityTreasureHunt,
    getActivityTreasureHunt,
    addActivityTreasureHunt,
    delActivityTreasureHunt,
    spuList,
    getskuList,
    pullOffIt,putOnIt
} from "@/api/business/luckybox/activity/activityTreasureHunt";
import { ElForm, ElTable } from "element-plus";
import { isStrings } from "@/utils/validate";

export default () => {
    const { proxy } = getCurrentInstance() as any;
    const state = reactive<activityTreasureHunt<activityTreasureHuntForm, activityTreasureHuntParams>>({
        // 查询参数
        queryParams: {
            current: 1,
            size: 50,
    ...
    const getList = () => { for (let key in queryParams.value) {
      if (queryParams.value.hasOwnProperty(key)) {
        ...
        loading.value = true;
        // prettier-ignore
        listActivityTreasureHunt(queryParams.value).then((response) => {
    ...
    const handleUpdate = (row: any) => {
        ...
        getActivityTreasureHunt(configId).then((response: any) => {
            form.value = response.data;
            form.value.treasureStartTime = parseInt(form.value.treasureStartTime);
            form.value.treasureEndTime = parseInt(form.value.treasureEndTime);
            open.value = true;
            title.value = "修改一元夺宝";
    ...
    const submitForm = async () => {
        await formRef.value?.validate((valid: boolean) => {
            if (valid) {
                if (form.value.id != "") {
                    addActivityTreasureHunt(form.value).then((response) => {
                        if (response.code === 200) {
  ...
  const handleStatusChange = async (val: any, row: any) => {
    ...
    const handleDelete = (row: any) => {
        ...
        proxy.setTableRowSelected(pageTableRef, row, true);
        // prettier-ignore
        proxy.$modal.confirm("是否确认删除一元夺宝编号为\"" + configIds + "\"的数据项?", "警告")
            .then(() => {
                return delActivityTreasureHunt(query.value);
            })
            .then((response: any) => {
                if (response.code === 200) {
                    getList();
