...
// prettier-ignore
import { listShopArr, getshopArr, addshopArr, delShopArr } from "@/api/business/luckybox/shop/shop";
...
export default () => {
  ...
  const getList = () => { for (let key in queryParams.value) {
    ...
    loading.value = true;
    // prettier-ignore
    listShopArr(queryParams.value).then((response) => {
  ...
  const submitForm = async () => {
    await formRef.value?.validate((valid: boolean) => {
      if (valid) {
        if (form.value.id != "") {
          addshopArr(form.value).then((response) => {
            if (response.code === 200) {
  ...
  const handleDelete = (row: any) => {
    ...
    proxy.setTableRowSelected(pageTableRef, row, true);
    // prettier-ignore
    proxy.$modal.confirm("是否确认删除店铺编号为\"" + configIds + "\"的数据项?", "警告")
            .then(() => {
                return delShopArr(query.value);
            })
            .then((response: any) => {
                if (response.code === 200) {
                    getList();
