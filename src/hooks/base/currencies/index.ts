...
import {
  ...
  delCurrencies,
  // allCurrencies,
} from "@/api/base/currencies";
import { isStrings } from "@/utils/validate";
import { formDefault, searchDefault } from "@/data/base/currencies";
...
export default () => {
  ...
  const handleShowDetail = (row: any) => {
    ...
    getCurrencies(configId).then((response: any) => {
  ...
  const handleUpdate = async (row: any) => {
    ...
    await getCurrencies(userId).then((response: any) => {
      if (response.code === 200) {
        form.value = response.data;
    ...
    await formRef.value?.validate((valid: boolean) => {
      if (valid) {
        ...
        if (form.value.id != "") {
          ...
          addCurrencies(obj).then((response: any) => {
            if (response.code === 200) {
  ...
  const handleDelete = (row: any) => {
    ...
    proxy.$modal
      ...
      .then(() => {
        ...
        return delCurrencies(query.value);
      })
      .then((response: any) => {
        if (response.code === 200) {
