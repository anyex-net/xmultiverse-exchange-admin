import { ElForm, ElTable, UploadInstance, ElUpload } from "element-plus";
import {
  ref,
  defineComponent,
  reactive,
...
import {
  ...
  delData,
  // allCurrencies,
} from "@/api/base/instrumentsCurrency";
import { isStrings } from "@/utils/validate";
import { formDefault, searchDefault } from "@/data/base/instrumentsCurrency";
...
export default () => {
  ...
  // 设置 ref 到 uploadRefs 中
  const setUploadRef = (el: UploadInstance, name: string) => {
    if (!uploadRefs.value) {
      uploadRefs.value = {};
    }
    if (el) {
      uploadRefs.value[name] = el;
    } else {
      delete uploadRefs.value[name];
    }
  };

  const iconUpload = ref<InstanceType<typeof ElUpload>>();
  ...
  //图片
  const handleUpdateImage = (fieldName: string) => {
    uploadPolicy()
        .then((res: any) => {
          if (res.code == 200) {
            ...
            nextTick(() => {
              const uploader = uploadRefs.value[fieldName];
              if (uploader) {
                uploader.submit();
              }

              setTimeout(() => {
                form.value[fieldName] = uploadParams.value.key;
              }, 2000);
...
//图片
// 图片变更时触发上传
  const iconChange = (file: any, fieldName: string) => {
    uploadParams.value.name = file.raw.name;
    handleUpdateImage(fieldName); // 传入字段名
  };
  const doInitPolicy = () => {
    handleUpdateForm1();
  };
